...
$username = $fullname = $petname = $petgender = $age = $address = $contact = $mail = $dob = $pass = '';
$url = "";
$UserError = $NameError = $petnameError = $petgenderError = $AgeError = $AddressError = $ContactError = $MailError = $DateError = $PassError = '';
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	{	
		...
		{  	$NameError = "petname cannot be empty!"; $flag=1;  }

		if(empty($_POST['petgender']))
		{  	$NameError = "Petgender cannot be empty!"; $flag=1;  }
		...
		if($flag==0)
		{
			...
			if(!validate_name($petname))
			{
				$PetnameError = "Invalid Name!"; $flag=1;
			}

			if(!validate_name($petgender))
			{
				$petgenderError = "Invalid Name!"; $flag=1;
			}
			...
			if($flag==0)
			{
				...
				$sql_insert = "INSERT INTO `patient`(`username`, `Name`, `petname`, `petgender`, `password`, `phone`, `email`, `address`, `dateOfBirth`) VALUES ('".$username."','".$fullname."','".$petname."','".$petgender."','".$hashedPass."','".$contact."','".$mail."','".$address."','".$dob."')";
...
<body>
	...
	<div class="regTop">
		...
		<p class="txt">CLIENT REGISTRATION</p>
	</div>
	...
	<section class="container">
		...
		<form  id="reg_form" action="<?php echo $_SERVER['PHP_SELF'].$url; ?>" method="POST">
			...
			<input class="inputField" type="text" name="petname" placeholder="Pet Name" value="<?php echo $petname ;?>">
			<span class="errorMsg"><?php echo $petnameError; ?></span>

			<!-- <input class="inputField" type="text" name="petgender" placeholder="Pet Gender" value="<?php echo $petgender ;?>"> -->
			<select class="inputField" name="petgender" value="<?php echo $petgender ;?>">
				<option value="#">Pet Gender</option>
				<option value="male">Male</option>
				<option value="female">Female</option>
			</select>
			<span class="errorMsg"><?php echo $petgenderError; ?></span>
			...
			<input class="inputField" type="password" name="pass" placeholder="Retype Password" value="<?php echo $pass ;?>">
			<span class="errorMsg"><?php echo $PassError; ?></span>
