...
$id = $username = $name = $petname = $dateOfBirth = $spec = $phone = $mail = '';
...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
	if(isset($_POST['logOut']))
	{
		...
		if($flag==0){
			$sql_update = "UPDATE patient SET `name`='".$name."',`petname`='".$petname."',`dateOfBirth`='".$dateOfBirth."',`phone`='".$phone."' WHERE ID = ".$_SESSION['userid']." ";
...
<body>
	...
	<section id="board">
		...
		<div>
			<p class="common_label">Pet Name</p>
			<input class="info common_inputField" type="text" name="u_pname" value="<?php echo $petname; ?>" readonly>
		</div>
		<div>
